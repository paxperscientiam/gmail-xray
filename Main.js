...
function CardSection(args) {
  ...
  .setContent(HtmlService
              .createTemplate('<strong>eMail X-Ray</strong> addon will always be free. ' +
...
function buildAddOn() {
  ...
  for (var i = 0; i < threads.length && i < MAX_THREADS; i++) {
    ...
    cardHeader = new CardHeader({subject:Message.subject,sender:Message.sender,count:count,age:age,status:msgStatus});        
    
    var card = CardService.newCardBuilder()
    .setHeader(cardHeader);
    
    for (var j = 0; j < count; j++) {
      var msg = new CardSection({message:Thread.message[j],index:j,count:count,link:Thread.link}).setCollapsible(false);
